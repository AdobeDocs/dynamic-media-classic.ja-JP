...
>[!NOTE]
>
>前のアセットまたは次のアセットボタンを選択して、同じフォルダ内の次のアセットまたは前のアセットを詳細ビューで表示することができます。これらのボタンは、詳細表示の右上隅にあります。
...
* **公開用の項目**  — 名前の左側にある **** 公開アイコンをクリックするか、 **[!UICONTROL ファイル/]** 公開または **[!UICONTROL ファイル/非公開をクリックします]**。
...
* **アセットの削除** -  **[!UICONTROL File]** / **[!UICONTROL Deleteをクリックします]**。

画像、画像セット、フォントなどの独立したファイルの場合は、詳細ビューで公開および編集ヒストリーを確認したり、ジョブの詳細を確認することができます。

次の表に、詳細表示で使用できる他のオプションのタイプを示します。

| アセットタイプ | 編集/調整 | プレビュー |
|--- |--- |--- |
| 画像 | 画像マップの追加<br>追加zoom  targetsCropSharpenCreate adjusted表示<br><br><br> | 可。ズームと画像プリセット |
...
| テンプレート | 編集 | 可 |
| ビデオ | 不可 | はい |
| ビネットとレンダリングされたビネット | 不可 | 画像が表示<br>ビネットの内容とレンダリング可能な要素の構造をXML形式で表示できます |
