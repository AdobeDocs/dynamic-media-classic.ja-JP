...
* キャンペーンアセットと製品情報を組み合わせて、クリックで買い物かごを合理化します。
...
* 世界中の 800 を超えるクライアント、月間 3.5 PB のトラフィック、管理に 5 億を超える資産を提供する実証済みのインフラストラクチャ。前年比 60 %のトラフィック増加。
...
60 GB のストレージが含まれます。 250 GB のブロック単位で追加ストレージを購入できます。 現在のストレージ割り当てを取得するには、契約の詳細を確認します。
...
+++**実行 [!DNL Dynamic Media] アップグレードが必要な機能がありますか？ で使用できる機能 [!DNL Dynamic Media] オン [!DNL Experience Manager] 資産？**
詳しくは、 [機能の比較](/help/upgrade-feature-comparison.md) ページ。
...
関連トピック [機能の比較](/help/upgrade-feature-comparison.md) ページ。
