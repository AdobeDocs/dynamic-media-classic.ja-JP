...
seo-title: 公開設定
description: 'null'
seo-description: 公開設定画面の設定は、デフォルトで、Dynamic Media ClassicサーバからWebサイトやアプリケーションにアセットを配信する方法を決定します。
...
公開設定画面の設定は、デフォルトで、Dynamic Media ClassicサーバからWebサイトやアプリケーションにアセットを配信する方法を決定します。設定が指定されていない場合、Dynamic Media Classicサーバーは、公開設定画面の初期設定の設定に従ってアセットを配信します。例えば、解像度属性が含まれていない画像を配信するように要求した場合、画像は Image Server 画面に指定されている初期設定のオブジェクト解像度設定で配信されます。
...
>[!NOTE]
>
>公開設定画面は、詳しい知識がある Web サイト開発者またはプログラマ向けに用意されています。Dynamic Media Classicでは、これらの画面の設定を変更したユーザーがScene7Publishing System、HTTPプロトコル標準および表記、基本的な画像処理技術に精通していることを前提としています。

## Image Server {#image-server}

Image Server 画面では、Image Server から画像を配信するための初期設定を指定します。次の 5 つのカテゴリの設定を指定できます（設定について詳しくは、Image Server 画面を参照してください）。

これらの設定は、Dynamic Media Classicサポート担当者の支援によってのみ変更してください。

**カタログ管理** これらの設定によって、Scene7Publishing Systemとカタログの関係が決まります。ほとんどのWebサーバーとは異なり、ダイナミックメディアImage ServerのURL呼び出しは、イメージファイルではなくマニフェストまたはカタログファイルに移動します。カタログファイル（eCatalog と混同しないでください）には、Image Server に公開されたすべてのコンテンツのリストと、各画像へのパスが含まれています。Digimarc ID がある場合は、「Digimarc ユーザ情報」セクションにユーザ情報を入力します。

**要求属性** これらの設定は、サーバから配信できる画像を制限します。

**デフォルトの要求属性** これらの設定は、画像のデフォルトの外観に関係します。

**共通サムネール属性** これらの設定は、サムネール画像の初期設定の外観と配置に関係します。

**カタログフィールドの初期設定** これらの設定は、画像の解像度と初期設定のサムネールの種類に関係します。

**カラーマネジメント属性** これらの設定によって、使用するICCカラープロファイルが決まります。

**互換性属性** この設定により、バージョン3.6で下位互換性を確保するために、テキストレイヤーの先頭段落と末尾段落の処理が可能になります。
...
>[!NOTE]
> 「ローカリゼーションサポート」オプションを設定するには、Adobe Dynamic Media Classicテクニカルサポートにお問い合わせいただくか、s7support@adobe.comまで電子メールをお送りください。
...
>[!NOTE]
>
>ロケールマップフィールドなど、Scene7Publishing Systemでローカリゼーションサポートオプションを設定する場合は、Adobe Dynamic Media Classicテクニカルサポートにお問い合わせください。または、s7support@adobe.com まで電子メールをお送りください。

Scene7 Publishing System（SPS）を使用する一般的な方法は、e コマース Web サイトの製品画像を管理することです。国際的なビジネスでは、同じ製品のアセットの表示が国によって異なるという課題に直面します。通常、この相違はメディア全体のごくわずかな部分に関するものです。このような相違に対処するため、国ごとにすべてのアセットをコピーして異なる部分だけを上書きするのは大変な作業で、マスターアセットは 1 つという概念に矛盾します。このようなアセットの相違によって、オーディオトラックが異なる国別のビデオから、製品に使用される電源コードの違いという微妙ながらも重要な違いまで対応できます。Dynamic Media Classicでは、基本的な検索メカニズムを使用します。Image Server が検索するアセットのサフィックスの順序を定義します（必要なロケールから始めます）。
...
| 画像セットとメディアセット | 一部のロケールでは、画像セット全体が異なることがあります——例えば、eCatalogが完全に異なる場合など——一般的には、ビューアによって処理されるロケール固有の画像セットからロケール固有の画像セットに変換します。一般的に、汎用セットの個々のIDがローカライズされたコンテンツを参照することがあります。例えば、電気製品のほとんどの画像をすべての言語で同一にし、コントロールパネルの画像だけ変えることができます。IS で ID が自動的に変換されるので、ロケール固有の画像セットを生成する必要はありません。 |
...
**カタログ管理** これらの設定によって、Scene7Publishing Systemとカタログファイルの相互関係が決まります。Dynamic Media Classic Render ServerのURL呼び出しは、カタログに対して行われます。この呼び出しは、サーバーから画像を配信するために呼び出します。

**Session Attributes** これらの設定これらの設定によって、エラーパラメータ、相対的な画像URLのURL、オブジェクトの重なりが許可されるかどうかが決まります。

**初期設定の素材属性** これらの設定により、画像の初期設定の解像度とシャープの設定が決まります。
