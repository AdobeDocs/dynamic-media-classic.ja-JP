...
# 利用可能：Adobe Dynamic Media Classicデスクトップアプリ {#dynamic-media-classic-desktop-app}
...
詳しくは、 [新しいAdobe Dynamic Media Classicログインエクスペリエンスが利用できるようになりました。](/help/new-ui-2020.md)
...
* の下での要求の難読化とロックモードの自動有効化 **[!UICONTROL 画像サービングをテスト]**.
   ...
   詳しくは、 [セキュアテストサービスのテスト](/help/testing-assets-making-them-public.md#testing-the-secure-testing-service).

* 認証メカニズムがAdobe Analyticsに更新されました。 新しい統合に関連している場合、または一部の Analytics 変数をDynamic Media Classicデスクトップアプリケーション内で更新する必要がある場合。

   詳しくは、 [Adobe Analyticsにログイン](/help/log-analytics.md) を参照してください。

## リリース20.21.2の修正点 {#minor-release}

* 20.21.1の既知の制限：の **[!UICONTROL サーバー]** ログイン画面のドロップダウンリストが空でした。
...
* ルートフォルダー内の多数のアセットが原因で、デスクトップアプリケーションが応答しなくなる。 (Windows®で修正。必要に応じてmacOSで作業 )。
...
1. Adobe Dynamic Media Classicデスクトップアプリケーション用の最新のインストーラーをダウンロードします。
   ...
   * 以前のバージョン (20.21.3) は、次の場所で入手できます。
...
1. の **[!UICONTROL 個人設定]** ページの **[!UICONTROL デスクトップ]** 見出しで、次のいずれかの操作を行います。
   * Dynamic MediaでキャッシュされたAdobeの画像ファイルをすべてコンピューターから削除するには、 **[!UICONTROL 画像キャッシュをクリア]**&#x200B;次に、 **[!UICONTROL OK]**.
   * Dynamic MediaでキャッシュされたAdobeファイルをすべてコンピューターから削除するには、 **[!UICONTROL アセットキャッシュをクリア]**&#x200B;次に、 **[!UICONTROL OK]**.
...
* この **[!UICONTROL サーバー]** Adobe Dynamic Media Classicデスクトップアプリケーションに更新した後、ドロップダウンリストが空になる20.21.1 — シナリオ：をインストールして、Adobe Dynamic Media Classic 20.20.1または20.20.2にログインし、アプリケーションを閉じます。 その後、Adobe Dynamic Media Classic 20.21.1に更新します。サインインしようとすると、 **[!UICONTROL サーバー]** ドロップダウンリスト **[!UICONTROL アカウントにサインイン]** ダイアログボックスが空です。 この問題を回避するには、次の手順を実行する必要があります。 [キャッシュを手動でクリアする](#clear-cache) （上記の手順を参照）。
...
![更新アイコン](/help/assets/refresh-icon1.png)
*更新アイコン*
