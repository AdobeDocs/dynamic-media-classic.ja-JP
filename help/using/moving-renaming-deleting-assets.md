...
参照パネルからアセットを移動、名前変更、削除できます。 さらに、テキストファイルを使用して複数のアセットを同時に削除できます。
...
ズームターゲット、画像属性および履歴エントリは、それらが派生するアセットを削除すると、完全に削除されます。 これらはアセットとともにごみ箱フォルダに移動されず、ごみ箱から元に戻すことはできません。

>[!IMPORTANT]
>
>一括削除は、リソース集約的な操作です。 同時に大規模な削除操作を実行するのではなく、一括削除を順次実行します。 Adobeでは、1時間あたりのアセット削除数を5000以下に制限することをお勧めします。 1時間あたり5000を超える量では、レート制限が発生する可能性があります。
...
アセットライブラリ全体で一度に複数のアセットを削除するには、削除するアセットをテキストファイルに一覧表示し、リストをAdobe Dynamic Media Classicに送信します。

Adobe Dynamic Media Classic IDのリストを作成し、テキスト（.txt）ファイルとして保存します。 各Adobe Dynamic Media Classic IDは、独自の行（その後にキャリッジリターン）にする必要があります。

リストを使用してアセットを削除するには、次の操作を行います。
