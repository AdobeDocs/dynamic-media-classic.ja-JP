...
| レポート | 情報 | 使用方法 |
| --- | --- | --- |
| 帯域幅 | | **重要**:「帯域幅」タブはサポートされなくなりました。 UI には引き続き表示されますが、帯域幅データは使用できず、すべての値が `0` のように表示されます。 |
| ストレージ | ストレージ使用量 | 会社がアップロードしたデータ量をトラッキングします。 |
| 画像コンテンツ | 合計ヒット数および画像配信量をリクエストタイプおよびサブタイプ別に分類して表示します。 | 非ビデオアセットの指標を含む、様々な画像タイプのリクエスト数とボリュームを追跡します。 |
