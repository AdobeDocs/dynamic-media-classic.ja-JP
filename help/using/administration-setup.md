...
1. 「**[!UICONTROL 保存]**」を選択して、ユーザー設定を完了します。
   ...
   新しいユーザーを追加すると、ウェルカムメールが送信されます。 このメールには、一時的なパスワードと、Adobe Dynamic Media Classicへのログイン方法が記載されています。

   ウェルカムメールが届かない場合は、Adobe Dynamic Media Classicのログインページ（https://s7sps1.scene7.com）に移動し、「**[!UICONTROL パスワードを忘れた場合]**」を選択してもらいます。 パスワードがリセットされ、新しい電子メールが送信されます。 ユーザーが電子メールを受信せず、迷惑メールフォルダーに振り分けられていない場合は、テクニカルサポートにお問い合わせください。
...
* **フィールド名でフィルター**:「**[!UICONTROL フィールドでフィルターを有効にする]**」を選択します。 次に、**[!UICONTROL フィールド名]** メニューを選択し、リストをフィルタリングする列を選択し、「文字をフィルター」メニューを選択してレターを選択します。 選択した文字は、列の1つのリストをフィルタリングします。 完全なリストを表示するには、**`Enable Filter By Field`** オプションの選択を解除します。

* **無効なユーザーを除外**：選択解除&#x200B;**[!UICONTROL 無効なユーザーを含める]**。 検索結果には、システム内のユーザだけが表示されます。 無効なユーザーがシステムおよび管理するアカウントから削除されました。

* **列見出しで並べ替え**：すべてのユーザーをステータス、名、姓、または電子メールで並べ替える見出しを選択します。 または、ユーザーの役割または有効/無効なステータスでソートします。
...
### データ表示オプションの設定 {#viewing-data-in-different-ways}
...
* **レポートのプレゼンテーションの種類を選択**: レポートの種類メニューで、**[!UICONTROL 概要]**、**[!UICONTROL 日単位]**、または&#x200B;**[!UICONTROL 月単位]**&#x200B;を選択して、概要、日単位、月単位でデータを整理します。 一部のレポートには、このオプションはありません。

* **期間を指定**: レポートの期間を定義するオプションを選択し、期間を定義した後で&#x200B;**[!UICONTROL 更新]**&#x200B;を選択します。

* **定義済み期間**：定義済みレポートメニューで、オプションを選択します。 例えば、先月からのデータを収集するには、「先月」を選択します。

* **カスタム期間**：定義済みレポートメニューで、**[!UICONTROL カスタム]**&#x200B;を選択します。 次に、**[!UICONTROL 開始月]** （または&#x200B;**[!UICONTROL 開始日]**）メニューの日付と、月数（または日数）メニューの日付を選択します。 ドメインレポートおよびビデオコンテンツレポートでは、レポート情報を収集するための特定の開始日と終了日を選択できます。
...
1. 必要に応じて次の操作を行います。
   ...
   * リンクを実際の画像にコピーするには、**[!UICONTROL URL コピーURL]**&#x200B;を選択します。 画像に移動してエラーを調査するには、このリンクをブラウザーウィンドウに貼り付けます。
...
* **時刻**：最初にエラーが報告された時刻から、最後にエラーが報告された時刻までの期間（過去24時間以内）。
