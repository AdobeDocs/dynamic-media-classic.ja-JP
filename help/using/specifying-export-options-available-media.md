---
title: Media Portal ユーザーが使用できる書き出しオプションの指定
...
管理者から権限が付与されている場合、Media Portal ユーザは画像を書き出すときにその形式を変更できます。 例えば、サイズ、ファイル形式および画質を変更できます。 書き出された画像を自動的に再フォーマットすることで、画像を個別に再フォーマットする必要がなくなり、効率が向上します。 また、管理者はプリセット、つまり事前に選択した画像形式の設定を作成できます。 画像の書き出し時にプリセットを使用すると、会社が指定した内容で画像の形式を変更できます。
...
1. 画像プリセットウィンドウで、必要に応じて次のオプションを選択します。
   ...
   * **Enable User-Defined Conversion**：選択すると、このオプションを選択すると、選択したAssetsを書き出しウィンドウの&#x200B;**[!UICONTROL サイズ]** ドロップダウンリストから「その他」を選択できます。 ユーザーは測定単位を選択し、目的の幅と高さを指定できます。 ユーザがこれらのファイルを書き出したり、ダウンロードしたりするとき、画像ファイルの形式が変更されます。
