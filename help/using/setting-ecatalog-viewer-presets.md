...
eCatalog ビューアプリセットによって、eCatalog ビューアのスタイル、動作、外観が決まります。 Adobe Dynamic Media ClassicにはeCatalog Viewer プリセットが含まれており、管理者アクセス権があればカスタムプリセットを作成できます。

プリセットを作成するには、新しいプリセットを作成するか、Adobe Dynamic Media Classicが提供するeCatalog Viewer プリセットから開始して、新しい名前で保存します。 印刷物を会社のカラーで表示し、スタイルを定義するには、独自のeCatalog ビューアプリセットを作成します。

カタログビューアプリセットには、ページナビゲーション、ズーム、検索、「テーマ」の選択に関する多くの設定が用意されています。 これらのコントロールの外観とビューアの表示方法は、eCatalog ビューアプリセットの選択によって異なります。

**eCatalog Viewer プリセットを設定するには（管理者アクセス権が必要です）:**

1. グローバルナビゲーションバーで、**[!UICONTROL 設定]** > **[!UICONTROL ビューアプリセット]**&#x200B;に移動します。
1. ビューアプリセット画面で、新しいプリセットを作成するか、既存のeCatalog ビューアプリセットから開始して、eCatalog ビューアプリセットを作成します。
...
1. （オプション） **[!UICONTROL 情報パネル設定]**&#x200B;で、**[!UICONTROL 情報サーバーURL]** オプションに、ビューアが置き換える次の特殊トークンを含めることができます。
...
1. （オプション）「**[!UICONTROL 情報パネル設定]**」の「**[!UICONTROL 応答テンプレート]**」ボックスに、Adobe Dynamic Media Classicで画像マップの情報を取得する際にエラーが発生した場合に表示するテキストを入力します。 例えば、会社とeCatalog名を受信しても、ロールオーバー識別子がない場合、このメッセージが表示されます。
...
1. 作成したeCatalog Viewer プリセットを使用してWeb ページにeCatalogを表示するには、**[!UICONTROL Default]**&#x200B;を選択します。
