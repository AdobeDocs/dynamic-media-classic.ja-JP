...
description: Adobe Dynamic Media Classicにラスター画像アセットをアップロードする方法を説明します。
...
>[!NOTE]
>
>Adobe Dynamic Media Classicでの新規または既存のUGC ベクターアセットのサポートは、2021年9月30日に終了しました。
...
* ラスター画像
  `https://s7ugc1.scene7.com/ugc/image?op=get_uploadtoken&shared_secret=fece4b21-87ee-47fc-9b99-2e29b78b602`この例では、共有秘密キーは`fece4b21-87ee-47fc-9b99-2e29b78b602`です
...
初期設定で、アップロードトークンは取得後 5 分（300 秒）で有効期限切れになります。 より多くの時間をリクエストするには、URLに`expires`と必要な時間を秒単位で含めます。 例えば、次のサンプル画像の URL では、1800 秒間有効なアップロードトークンを取得します。

```as3
https://s7ugc1.scene7.com/ugc/image?op=get_uploadtoken&shared_secret=fece4b21-87ee-47fc-9b99-2e29b78b602&expires=1800
```

画像に対する正常な応答は、次のようになります。
...
| 期限切れ | オプション | アップロードトークンの有効秒数。 デフォルトは300秒です（指定しない場合）。 |
...
`file_limit` パラメーターは、ファイルサイズの制限をバイト単位で指定します。 `file_exts` パラメーターは、アップロードに使用できるファイル名拡張子を指定します。 この 2 つのパラメータはオプションです。

ファイルサイズ制限とファイル名拡張子に適用されるグローバル制限は、アプリケーション内で設定されます。 リクエストで送信した内容がグローバル制限のサブセットである場合は、それが尊重されます。 グローバル制限は次のとおりです。
...
| --- | --- | --- |
| `op` | 必須 | アップロード |
| `upload_token` | 必須 | 会社に関連付けられている共有秘密鍵のトークンをアップロードします。 |
