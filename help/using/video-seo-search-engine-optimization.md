...
SEOとは、検索エンジンからweb サイトへのトラフィック量を増加させるプロセスのことです。 検索エンジンはテキストベースのコンテンツに関する情報を収集するのに効果的ですが、動画に関する情報を適切に処理することはできません。 提供できるようになりました。

検索エンジンに動画の説明を提供するには、Adobe Dynamic Media Classic Video SEOを使用して動画のメタデータを適用します。 Adobe Dynamic Media Classicでは、ビデオサイトマップとmRSS フィードを作成できます。 これらの標準XML ファイルは、検索エンジンにビデオ情報を送信するために使用されます。

* **ビデオサイトマップ**: サイト上のビデオコンテンツの場所と内容をGoogleに正確に通知します。 動画はGoogleで完全に検索できます。 例えば、ビデオサイトマップでビデオの実行時間とカテゴリを指定できます。 ビデオサイトマップについて詳しくは、[&#x200B; ビデオサイトマップとビデオサイトマップの代替案](https://developers.google.com/search/docs/crawling-indexing/sitemaps/video-sitemaps?visit_id=637558394348624754-567115452&rd=1)を参照してください。
...
Adobe Dynamic Media Classicでは、各動画に保存されているメタデータから動画サイトマップとmRSS フィードを生成できます。 ビデオサイトマップおよび mRSS フィードを作成する場合は、ビデオファイルから抽出するメタデータフィールドを指定します。 検索エンジンがweb サイト上の動画へのトラフィックをより正確に誘導できるように、動画を検索エンジンに説明します。

>[!NOTE]
>
>ビデオサイトマップまたはmRSS フィードを作成する前に、検索エンジンがXML ファイルに必要とするフィールドと、これらのフィールドの構造化方法を決定します。 ビデオサイトマップまたは mRSS フィードを正常に作成するには、検索エンジンの要件を満たす必要があります。
...
**[!UICONTROL 一般設定]**&#x200B;領域で、ビデオサイトマップ、mRSS フィード、またはその両方を生成するかどうかを選択します。 メタデータフィールドを入力フィールドにマッピングするには、**[!UICONTROL 生成設定]**&#x200B;領域を使用します。
...
* **自動モード**: Adobe Dynamic Media Classicは、毎日1つのビデオサイトマップ、Media RSS （mRSS）フィード、またはその両方を自動的に生成します。 「**[!UICONTROL 公開用にマーク」]** オプションを選択して、Adobe Dynamic Media Classicが公開用に生成するXML ファイルにマークを付けます。

  * **公開用にマーク**&#x200B;生成されたXML ファイルを公開用にマークします。

* **手動モード**: ビデオ検索エンジンの最適化画面で「**[!UICONTROL 生成]**」または「**[!UICONTROL 保存と生成]**」を選択すると、Adobe Dynamic Media Classicによってビデオサイトマップ、Media RSS （mRSS） フィード、またはその両方が生成されます。 これらのオプションも設定します。
...
「生成設定」領域には、ビデオサイトマップ、mRSS フィード、またはその両方の入力フィールドが一覧表示されます。 メタデータパネルには、メタデータフィールドの名前が表示されます。 生成設定領域を使用して、入力フィールドをメタデータフィールドに割り当てます。 ビデオサイトマップやmRSS フィードのメタデータを取得するようにAdobe Dynamic Media Classicを設定します。
...
1. 次のいずれかの操作を行います。
   ...
   * ファイルを保存して生成するには、**[!UICONTROL 保存して生成]**&#x200B;を選択します。
     ...
     XML ファイルが生成され、ジョブログに記録されます。 ビデオサイトマップ（ビデオサイトマップ）とMedia RSS （mRSS）フィード（mrss フィード）は、会社のルートフォルダーに保存されます。
...
## ビデオサイトマップとmRSS フィードのファイルを検索エンジンに送信する {#submitting-video-sitemap-and-mrss-feed-files-to-search-engines}
...
ビデオサイトマップまたはMedia RSS （mRSS）フィードファイルを検索エンジンに送信するには、これらのURLの1つを検索エンジンのweb マスターツールにコピーします。
