---
title: スウォッチセットにズームターゲットと画像マップを含める
description: Adobe Dynamic Media Classicのスウォッチセットにズームターゲットと画像マップを含める方法について説明します。
contentOwner: Rick Brough
content-type: reference
products: SG_EXPERIENCEMANAGER/Dynamic-Media-Classic
geptopics: SG_SCENESEVENONDEMAND_PK/categories/swatch_sets
feature: Dynamic Media Classic,Viewers
role: User
exl-id: 1eaffe17-1fa3-4726-b0a1-369fabb9ee7e
topic: Content Management
level: Intermediate
autotag-review: '2026-05-13T20:01:32.867Z'
TQID: 'https://experienceleague.adobe.com/jpI0LZAbj3AUCT7UMn0op4EOJGaz2V8JnzZNxz9W2jQ'
product_v2: id: beaff0dd-a904-4c6b-8290-b527cd877d75
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588
source-git-commit: 0117afba420d2d4dce072b78a35e44328e72c950
workflow-type: tm+mt
source-wordcount: 165
ht-degree: 0%

---

# スウォッチセットにズームターゲットと画像マップを含める{#including-zoom-targets-and-image-maps-in-swatch-sets}

スウォッチセット内の画像に対してズームターゲットと画像マップを定義すると、これらのターゲットと画像マップはスウォッチセットビューアに表示されます。 例えば、スウォッチセットビューアで画像マップを選択し、アイテムに関する情報を含むweb サイト上の新しいページを開くことができます。 ユーザーは、スウォッチセットビューアでターゲットサムネール画像を選択し、画像の一部に自動的にズームできます。

ユーザーがズームターゲットと画像マップを使用できるようにするには、スウォッチセット内の画像のズームターゲットと画像マップを作成します。 この操作は、スウォッチセット画面または参照パネル（詳細表示）の画像マップまたはズームターゲットツールで実行できます。

>[!MORELIKETHIS]
>
>* [ ガイド付きズームのズームターゲットを作成](creating-zoom-targets-guided-zoom.md#creating_zoom_targets_for_guided_zoom)
>* [画像マップの作成](creating-image-maps.md#creating_image_maps)
