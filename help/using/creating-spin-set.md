...
スピンセットに含まれる画像が多ければ多いほど、画像スピニング効果の画質が高くなります。 ただし、セット内の画像が多いほど、画像の読み込みに時間がかかるようになります。 Adobe Dynamic Media Classicでは、スピンセットで使用する画像を撮影する際に、次のガイドラインをお勧めします。
...
* 商品の詳細が明らかになり、焦点が合っていることを確認します。
* 衣類のスピン画像を撮影する場合は、マネキンまたはモデルを使用します。 多くの場合、マネキンは（ガラスマネキンを使用して）マスクされているか、または様式化されたマネキン/ドレスフォームが画像に示されている。 角度の数を定義することで、モデル上のスピンセットを作成できます。 床の上に各角度をマークして、モデルをガイドして自分自身を配置し、各ショットの方向を見ることができます。

## スピンセットの作成 {#create}

Adobe Dynamic Media Classicでスピニングセットを作成または作成する順序は重要です。 スピンセットページのグリッドに画像をドラッグ&amp;ドロップする際のアセットの順序に応じて、スピンセットは特定の方向に回転します。 そのため、ビルダーに表示される順序によって、ユーザーがマウスポインターを移動したり、指を左から右に移動したりしたときに、アセットがどのように回転するかを決めます。
