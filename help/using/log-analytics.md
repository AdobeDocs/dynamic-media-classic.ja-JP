...
自分がAdobe AnalyticsのWeb サービスアクセスグループのメンバーであることを確認します。 Adobe Analytics レポートを設定し、Adobe Analytics レポート変数をAdobe Dynamic Media Classic イベントに一致させるには、ログインする前に、この手順を完了してください。 このグループのメンバーは、指定されたレポートスイート内のすべてのレポートにアクセスできます。 インターフェイスで設定された権限に関係なく、Experience Cloud Web Services APIを使用してこのアクションを実行します。 グループにメンバーを追加するには、Adobe Analyticsで、**[!UICONTROL 管理ツール]** > **[!UICONTROL ユーザー管理]** > **[!UICONTROL グループを編集]**&#x200B;に移動します。

ログインすると、最新のビデオ分析実装を使用するためにExperience Cloud組織IDを入力するオプションがあります。 IDを入力しない場合でも、ビデオレポートは機能します。 ただし、Adobe Dynamic Media Classic以外のクライアントのデータとデータが正しく統合されない場合があります。
...
Dynamic Media ClassicとAdobe Analytics OAuthを統合します。 Dynamic Media ClassicとのAdobe Analytics OAuth統合は、通常、ユーザーごとに1回のみ実行されます。
...
1. 次のいずれかのオプションを選択します。
   ...
   * ページの上部で、「**[!UICONTROL プロジェクト]**」タブを選択します。 ページの右隅付近にある「**[!UICONTROL 新しいプロジェクトを作成]**」を選択します。
...
1. **[!UICONTROL 資格情報の詳細]**&#x200B;で、次の操作を行います。
   * **[!UICONTROL クライアント ID]**&#x200B;で、**[!UICONTROL コピー]**&#x200B;を選択して値をコピーします。 この値は、Dynamic Media Classic デスクトップアプリケーションの後続のAnalytics設定に必要です。
   * **[!UICONTROL クライアントシークレット]**&#x200B;で、**[!UICONTROL クライアントシークレットの取得]**&#x200B;を選択して、関連付けられた値を表示します。 値をコピーするには、**[!UICONTROL コピー]**&#x200B;を選択します。 この値は、Dynamic Media Classic デスクトップアプリケーションの後続のAdobe Analytics設定に必要です。

## Adobe Dynamic Media ClassicでのAdobe Analyticsの設定 {#configure-analytics-in-dmc}

>[!NOTE]
>
>Dynamic Media ClassicでのAdobe Analyticsの初期設定の後、設定を繰り返す必要があるのは次の場合だけです。
>
...
1. ダイアログボックスの右下隅にある「**[!UICONTROL ログイン]**」を選択し、Adobe IMS（Identity Management サービス）ログインを実行します。
   ...
   ログインに成功すると、Adobe Analytics ログインダイアログボックスが再度表示され、利用可能な企業が入力した&#x200B;**[!UICONTROL COMPANIES]** ドロップダウンリストが表示されます。

1. **[!UICONTROL COMPANIES]** ドロップダウンリストから、会社を選択します。

   会社を選択すると、選択した会社で使用可能なレポートスイートが入力された&#x200B;**[!UICONTROL SUITES]** ドロップダウンリストが表示されます。
