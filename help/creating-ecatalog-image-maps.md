...
1. 以下の手順に従って、長方形または多角形の画像マップを画像マップ領域で描画します。
   ...
   * **多角形マッ**&#x200B;プ多角形画像マップツールを選択し、画像の周囲を必要な回数だけクリックします。 クリックすると、画像マップの境界線が描画されます。
   画像マップを描画した後、Dynamic Media Classicは画像マップリストで画像マップに名前を割り当てます。 この名前を作成するために、作業対象のeCatalogページの名前に連番が追加されます。
...
* **位置の調整パンツ**&#x200B;ールを選択し、マップの境界線上ではなく近くにポインタを移動します。 ポインタが 4 方向の矢印アイコンに変わったら画像マップ全体を新しい場所に移動します。
...
* **画像マップの削除**&#x200B;パンツールを選択し、画像マップをクリックして選択し、「削除」をクリック **します**。

   To remove all Image Maps from an eCatalog, click the **Order Pages** tab, and then click **Clear Maps**.

* **画像マップの重なり合った**&#x200B;場合の処理画像マップをドラッグして、画像マップリスト上の画像マップの順序を変更
...
eCatalog 内のすべての画像マップを別の eCatalog にコピーできます。画像マップのコピーというこの方法により、同じ eCatalog の各国語版で容易に画像マップをコピーすることができます。コピーが正常に完了するように、同じページ数と同じ画像を持つeCatalog間でコピーすることをお勧めします。
...
ターゲットeCatalog（画像マップのコピー先のeCatalog）のページ数や画像数が異なり、サイズが異なる場合は、警告メッセージが表示されます。 警告を無視して画像マップをコピーするには、「続行」をクリックします。
