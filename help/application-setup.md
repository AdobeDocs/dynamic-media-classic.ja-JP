...
description: ダイナミックメディアクラシックのアプリケーション領域を設定する方法を説明します。
seo-description: ダイナミックメディアクラシックのアプリケーション領域を設定する方法を説明します。
...
アプリケーション設定ページを使用して、一般設定の入力、画像プリセットの作成、ビデオエンコーディングプリセット、ビューアプリセットの作成、初期設定のビューアおよびメタデータの定義を行うことができます。 また、バッチセットプリセットを設定して、2D スピンセット（など）や公開設定、ビデオ SEO 設定を自動作成することができます。
...
アカウントの作成時に、Dynamic Media Classicは、割り当てられたサーバーを自動的に会社に提供します。 これらのサーバは、Web サイトとアプリケーションの URL 文字列を生成するのに使用されます。これらの URL 呼び出しは、アカウントに固有です。

[セキュアテストサービスのテスト](testing-assets-making-them-public.md#testing_the_secure_testing_service)も参照してください。

**Published Server Name** ：このサーバーは、お使いのアカウントに固有のシステム生成URL呼び出しで使用されるライブCDNサーバーです。 Dynamic Media Classicのサポート技術者による指示がない限り、このサーバー名を変更しないでください。

**接触チャネルサーバ名** ：このサーバは、品質保証テストにのみ使用されます。 Dynamic Media Classicのサポート技術者による指示がない限り、このサーバー名を変更しないでください。

**AGM Server Name** このサーバーは、Web-to-Printテンプレートで使用されます。 このサーバは会社全体に適用されます。Dynamic Media Classicのサポート技術者による指示がない限り、このサーバー名を変更しないでください。

**Test&amp;Server Name** Your Test&amp;Server URL（.comを含む）。 このURLの取得方法については、ダイナミックメディアクラシックとターゲットクラシックの統合を参照してください。

**iOSストリーミングサーバ名** :Dynamic Media Classic iOSストリーミングサーバのURL。 このサーバは、HTTP プロトコルを使用して iOS ベースのデバイスにストリーミングビデオを配信します。

**プログレッシブビデオサーバ名** ：ダイナミックメディアクラシックプログレッシブビデオサーバのURL。 このサーバは、HTTP プロトコルを使用してプログレッシブビデオを配信します。

**非公開アセットのURLを表示** ：公開済みかどうかに関係なく、アセットをプレビューする際にURLを表示する場合は、このオプションを選択します。 アセットが公開されていない場合は、URL は機能しません。ただし、この URL を予定や組織的な目的で使用することができます。
...
**「生成されたコンテンツをフォルダに表示** 」は、アセットから生成されたコンテンツを表示します。 例えば、PDFファイルがアップロード時にラスタライズされると、Dynamic Media Classicは元のPDFの各ページに対して1つの画像を作成します。 「生成されたコンテンツを表示」が選択されている場合、元の PDF がアップロードされたときに生成された各画像が、PDF がアップロードされたフォルダの PDF とともに表示されます。
...
>[注意]
>
>Scene7 Publishing Systemを使用してビデオアセットをアップロードおよびエンコードしなかった場合、このオプションが選択されていなくても、Dynamic Media Classicは個々のエンコードされたビデオをすべて表示します。

**[サブフォルダを更新]ボタン** ：サブフォルダの[更新]ボタンの表示/非表示を切り替えます。

**ダイナミックメディアクラシックFTPアカウント**

**サーバーは** 、FTPアカウントサーバーをリストします。
...
**Default Image Mapping HREF** ：画像マッピングHREF列に使用するデフォルトのURLを定義します。 この URL は、新しい画像マップを作成するときに表示される初期設定の URL です。

**デフォルトの画像マッピングテンプレート** ：画像マッピングHREFテンプレートのデフォルトのJavaScriptを定義します。 画像マップをクリックしたときに実行されるカスタムコードをここに設定できます。

**アプリケーションのその他の設定**

**ごみ箱内の警告アセットを** 、ごみ箱から7日以内に自動的に削除されます。 ごみ箱内のアセットが完全に削除される 4 日前に会社の管理者に通知を送信する場合、「ごみ箱の項目が自動的に削除される前に電子メールを送信する」を選択します。詳しくは、ごみ箱フォルダの管理を参照してください。
...
以前に画像をアップロードしてから元のファイルを変更（または置き換え）した場合、選択した「上書き」オプションでは、画像をダイナミックメディアクラシックで置き換える方法を指定します。 画像に関する情報は変わりませんが、古い画像は新しい画像に置き換わります。フォルダーに、まだダイナミックメディアクラシックにない画像が含まれている場合は、これらの画像が追加されます。
...
実稼働サーバーからの画像へのアクセスに使用されるDynamic Media Classic IDは、画像ファイル名から取得されます。 既存のファイルを置き換える場合や、画像へのアクセスに使用するダイナミックメディアクラシックIDの場合は、ファイル名に大文字と小文字を使用することが重要です。 同じ画像で大文字と小文字のみが異なるダイナミックメディアクラシックIDは使用しないように、Dynamic Media Classicにアップロードする前に、ファイル名に大文字と小文字を使用するのが正しいことを確認してください。
...
**[幅]と[高さ** ]画像を配信するサイズをピクセル単位で入力します。
...
* JPEG オプション
   ...
   **画質**
   ...
   JPEG 圧縮レベルを制御します。この設定は、ファイルサイズと画質の両方に影響を与えます。JPEGの画質スケールは1 ～ 100です。
...
**シャープ** 「シンプルなシャープを有効にする」オプションを選択すると、すべての拡大・縮小が行われた後に、基本的なシャープフィルタが画像に適用されます。 シャープにより、異なるサイズで画像を表示したときに生じるぼかしを補正できます。
...
**B-Linear** 最も高速な再サンプリング方法。一部のエイリアシングアーチファクトが目立ちます。

**バイキュービック** :Image ServerでのCPU使用量が増加しますが、エイリアシングアーチファクトが少ないシャープな画像が得られます。

**Sharp2** Bi-Cubicオプションよりも多少シャープな結果を得られますが、Image ServerのCPU使用率はさらに高くなります。

**[トリリニア]** （可能な場合）高い解像度と低い解像度の両方を使用します。エイリアシングが問題となる場合にのみ推奨されます。 この方法では、周波数の高いデータが削減されるため JPEG のサイズが小さくなります。
...
**[カラースペース** ]イメージが作成されたスペース(通常はRGB（元の値）または輝度スペース（適用度）)のどちらを使用するかを指定します。

**カラー** 次のオプションを選択します。

**出力カラープロファイル** 「初期設定を使用」またはScene7 Publishing Systemで使用可能なICCカラープロファイルの1つを選択します。

詳しくは、[ICC プロファイル](icc-profiles.md#icc_profiles)も参照してください。

**[レンダリングインテント** ]カラーオプションの既定のレンダリングインテントを上書きする場合は、オプションをプロファイルします。 このオプションは、初期設定の ICC プロファイルのいずれかがカラー変換のターゲットカラースペースであり、出力デバイス（プリンタまたはモニタ）がこのプロファイルによって特徴付けられ、指定したマッチング方法がこのプロファイルで有効な場合に使用されます。

**埋め込みプロファイル** Adobe® Photoshop®でこの画像を開いた場合にこの画像が使用されるように、このオプションをプロファイルします。

**[印刷解像度** ]この画像を印刷する解像度を選択します。デフォルトは72ピクセルです。

**URL修飾子** ：設定ではなく、画像プリセットを定義するURL修飾子を指定する場合は、ここに修飾子を入力します。

**サンプル画像URL** リストには、追加または編集する画像プリセットと共に画像を配信するためにDynamic Media Image Serverで使用される「生の」URL文字列が含まれます。 この URL 文字列は、プリセットを追加画面またはプリセットを編集画面で選択したすべての形式設定をエンコードします。
...
ダイナミックMedia Classicオファーアダプティブビデオエンコーディングプリセット。 これは 16:9 アダプティブビデオプリセットと 4:3 アダプティブビデオプリセットの両方を組み合わせて 1 つのグループにした、プリセットのマスターリストです。これらの定義済みプリセットは、最も一般的なエンコーディング設定を反映しており、ターゲットとなっている携帯端末、タブレットおよびデスクトップでの再生用に最適化されています。
...
* 次のアダプティブビデオエンコーディングプリセットまたはシングルエンコーディングプリセットを選択します。
   ...
   **16:9 アダプティブビデオ**
   ...
   デスクトップ、モバイル(iPhone、iPad、Android)およびタブレット(iPad、Android)向けの16:9の縦横比のビデオを作成し、ビューアの接続速度に最も適した解像度とビットレートで最適化します。

   **4:3 アダプティブビデオ**

   デスクトップ、モバイル(iPhone、iPad、Android)およびタブレット(iPad、Android)向けに、Viewerの接続速度に最も適した解像度とビットレートで最適化した4:3の縦横比のビデオを作成します。
   ...
   **アダプティブビデオエンコーディング（16:9 または 4:3）**
   ...
   デスクトップ、モバイル(iPhone、iPad、Android)、タブレット(iPad、Android)向けの16:9と4:3の縦横比のビデオを、Viewerの接続速度に最も適した解像度とビットレートで作成します。
...
**エンコードされたファイルのサフィックス** 。識別のためにエンコードされたビデオファイルに付加されるサフィックスです。
...
詳しくは、 [『Adobe Viewers Reference Guide](https://marketing.adobe.com/resources/help/en_US/s7/viewers_ref/c_html5_viewers_about.html) 』のHTML5ビューアについてを参照してください。

詳しくは、ダ [イナミックメディアクラシックビューアプリセットの互換表を参照してくださ](application-setup.md#scene7_viewer_preset_compatibility_matrix)い。
...
**eCatalogビューア** ：印刷されたカタログを読み上げる操作をシミュレートします。 ページ間の移動、ページ上の項目のズームイン/ズームアウト、画像マップを使用したページ上の項目の詳細の表示、カタログの検索を行うことができます。 詳細情報を表示する情報パネルを含めたり、また画像マップ領域に有効な rollover_key 属性がある場合は画像マップ項目を含めたりすることもできます。情報パネルを含めるには、eCatalog ビューアプリセットウィンドウの情報パネルの設定パネルで、情報サーバの URL を指定します。
...
**スピンセットビューア** ：複数の表示の画像を使用して、オブジェクトを回転させ、異なる辺や角度を調べることができます。
...
**ズームビューア** オファーには、次の3種類のズームビューアから選択できます。
...
**Flashビューアの提供終了の通知**:2017年1月31日、Adobe Scene7 Publishing SystemはFlashビューアプラットフォームのサポートを正式に終了しました。
...
詳しくは、 [Adobe Viewers Reference Release Notesを参照してください](https://marketing.adobe.com/resources/help/en_US/s7/viewers_ref/)。
...
ダイナミックメディアクラシックは、MP4 H.264ビデオのモバイルビデオ再生をサポートしています。
...
**プリセットの追加** ビューアプリセット追加ダイアログボックスでクリ追加ックし、選択を行います。
...
**並べ替えプリセット** 列見出し（「アクティブ」、「種類」、「プリセット」または「プラットフォーム」）をクリックして、列のリストを並べ替えます。 列見出しをもう一度クリックすると、リストが降順（または昇順）に並べ替えられます。
...
1. ビューアプリセット画面で、ビューアプリセットを追加または編集します。
   ...
   **追加** ：ツー追加ルバーのをクリックします。 ビューアプリセットを追加ダイアログボックスで、プラットフォームを選択し、リッチメディアアセットタイプを選択します。

   ビューアプリセットの作成が完了したら、「**名前を付けて保存**」をクリックします。

   **追加するには、既存のビューアプリセットを使用します** 。表で、ビデオビューアプリセットを選択し、ツールバーの「編集」をクリックします。
...
SPSの初期設定の初期設定の初期設定のビューアプリセットCSSファイルは、すべて、上に配置されたアセットを指す相対画像サービングパスを使用しま `Scene7SharedAssets`す。 例えば、次の図は、にあるビューアプリセットCSSファイル内の画像アセットの相対パスで `Scene7SharedAsset`す。ただし、 `.s7videoviewer .s7fullscreenbutton[state][selected] { background-image: url(/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha); }`Viewer CSSファイルを独自のサイトでホストする場合は、独自の環境ー内のImage Serverへの明示的なパスを使用して、これらの相対画像パスを解決する必要があります。 説明のため、上の相対パスを明示的なパスに更新する場合、次のようになります。は、 `https://s7d1.scene7.com` image serverへの直接パスです。 `https://s7d1.scene7.com/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha`
...
1. 選択したアセットを書き出しダイアログボックスで、「**書き出しを送信**」をクリックします。
   ...
   書き出し後、CSSファイルが取得されます。 そのファイルをダウンロードして展開します。
...
Dynamic Media Classicには定義済みのメタデータ表示が付属しており、管理者は独自のメタデータ表示を作成して、ユーザがメタデータを入力する際に選択できるようにすることができます。

### メタデータビューの作成 {#creating-a-metadata-view}

1. Click **Setup** > **Application Setup** > **Metadata** > **Metadata Views**.
1. **「追加」**&#x200B;をクリックします。
1. 「プリセット名」テキストフィールドに、プリセットの名前を表示します。
...
1. メタデータプリセット画面で、次のいずれかの操作を実行します。
   ...
   * To create a preset, click **Add**. 「メタデータ表示名」テキストフィールドにプリセットの名前を入力し、「メタデータ表示 **」をクリックして、ドロップダウンリストからを選択します(メタデータテンプレート** の表示を参照 [](application-setup.md#metadata_views))。
...
Media Portal 管理者または会社管理者が、ユーザ定義のカスタムメタデータフィールドを作成できます。カスタムフィールドは、Scene7 Publishing Systemのアセットを整理するのに役立ちます。 必要に応じて、フィールドをアクティブとしてマークすることができます。 アクティブ化すると、これらのカスタムメタデータフィールドの名前は、詳細ビューのメタデータパネルに表示されます。ユーザは、ユーザ定義メタデータフィールドに情報を入力して、アセットの説明を追加することができます。また、ユーザ定義メタデータフィールドを検索条件にすることもできます。

ユーザ定義メタデータフィールドの効果的な使用方法の 1 つとして、特定の発売（販売）でアセットのアクティブ化の時間を遅らせることができます。「日付」タイプに基づいて「アクティベーション」フィールドを定義 *します*。 Then, using the **Metadata** panel in **Detail** view or **File** > **Edit Info**, you can specify when the asset is activated. Scene7 Publishing System は、アセットの公開ステータスと公開ヒストリーを確認します。公開ステータスがアクティベーション時間内でない場合は、「未公開」と表示されます。
...
1. カスタムフィールドダイアログボックスで、目的のオプションを設定します。
   ...
   **入力** ：メタデータフィールドに入力できる情報の種類を定義するオプションを選択します。
   ...
   **型指定なし** （下位互換性）。 このオプションを選択してはいけません。

   **デフォルト値** （オプション）ユーザーがフィールドに入力する可能性が最も高い値を入力します。 入力した値が、作成したフィールドの初期設定値になります。

   **適用先** （オプション）メタデータフィールドを特定の種類のアセットにのみ適用する場合は、アセットの種類を選択します。
...
**フィールドの編集** フィールドを選択し、「編集」をクリッ **クします**。
...
1. 設定/ア **プリケ** ーション設定 **を選択し、「ファイルの**&#x200B;最適化」を選択します&#x200B;****。
...
会社のアクティブなバッチセットプリセットはすべて、アップロードオプションを設定ダイアログボックスに表示されるので、各アップロードセッション中に適用するプリセットを指定できます。会社の管理者は、すべてのアクティブおよび非アクティブなバッチセットプリセットを確認できます。ファイルをアップロードすると、アクティブなプリセットで定義された命名規則に一致するすべてのファイルが含まれたセットが、Dynamic Media Classicによって自動的に作成されます。
...
Dynamic Media Classicは、バッチセットプリセットを使用して、一部の共通の情報やコンテンツを画像のセットに共有するアセットを整理し、ビューアに表示します。 バッチセットプリセットレシピは、Dynamic Media Classicでスケジュールしたアセットの読み込みジョブと同時に自動的に実行されます。
...
1. 「シーケンスの順序」では、セットをダイナミックメディアクラシックでグループ化した後の画像の順序を定義します。 初期設定では、アセットはアルファベット順に並んでいます。ただし、カンマ区切りの正規表現リストを使用して順番を定義できます。
1. 命名規則と作成オプションの設定では、アセットの命名規則で定義したベース名にサフィックスとプリフィックスを指定します。また、Dynamic Media Classicのフォルダ構造内で画像セットを作成する場所も定義します。
