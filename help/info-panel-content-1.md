...
1. （オプション）「デフォルト応答」ボックスに、Dynamic Mediaマップの情報を取得する際にエラーが発生した場合に表示するテキストを入力します。 例えば、システムで会社名と画像セット名は取得されるが、ロールオーバー識別子は取得されない場合に、このメッセージがユーザに表示されます。
...
### 外部でホストされている場所からCSVコンテンツを画像セットに読み込む {#import-csv-content-into-image-sets-from-an-externally-hosted-location}
